...
def StoreData():
    for file in os.listdir(os.path.join(BaseDir, "InstrumentAudioFiles")):
        filename = os.fsdecode(file)
        name = os.path.join(BaseDir, "InstrumentData", f"{filename.rsplit('.', 1)[0]}.pkl")
        ...
        full_filename = os.path.join(BaseDir, "InstrumentAudioFiles", filename)
        if filename.endswith((".aiff", ".aif", ".wav", ".flac", ".mp3")):
            print(filename)
            try:
                if filename.endswith(".mp3"):
                    audio = AudioSegment.from_mp3(full_filename)
                    data = np.array(audio.get_array_of_samples())
                    samplerate = audio.frame_rate
                else:
                    data, samplerate = sf.read(full_filename)
